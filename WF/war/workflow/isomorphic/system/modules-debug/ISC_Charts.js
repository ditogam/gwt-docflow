...
if (window.isc && isc.version != "v10.0p_2014-09-18/EVAL Development Only") {
    isc.logWarn("SmartClient module version mismatch detected: This application is loading the core module from "
        + "SmartClient version '" + isc.version + "' and additional modules from 'v10.0p_2014-09-18/EVAL Development Only'. Mixing resources from different "
...
setupChart:function(){
    ...
    for(var i=0;i<facetDataLength;++i){
        var record=facetData[i];
        if(record!=null){
...
getDataRecord:function(facetValues,multiple){
    ...
    if(!this.inlinedFacet){
        if(facetValues!=null&&this._shouldOrderFacetData()){
            var index=this._getTempIndex();
            this._getFacetDataIndex(facetValues,false,index);
            if(multiple){
                return this._facetDataMultiArray.slice(index).toArray(true);
            }else{
                return this._facetDataMultiArray.get(index);
            }
        }else{
            return(multiple?
...
getMinValue:function(metricId,recalc,context,stacked,considerPartialSums,logScale){
    return this.getMaxValue(metricId,recalc,context,stacked,considerPartialSums,logScale,false);
},
getMaxValue:function(metricId,recalc,context,stacked,considerPartialSums,logScale,wantMax){
...
,isc.A._getAxisBoundaryValues=function isc_FacetChart__getAxisBoundaryValues(primary,minValue,maxValue,gradationIncrement,context){
    ...
    if(numericDataAxis){
        ...
        if(proportional){
            fixedBase=true;
            base=0;
        }else if(isDateValued?isc.isA.Date(context[prop]):isc.isA.Number(context[prop])){
            fixedBase=true;
            base=context[prop];
        }
    }
    ...
    if(!numericDataAxis){
        if(primary||this._getPointSizeMetric()!=null){
            ...
            if(isDateValued){
                ceil=isc.DateUtil._max(base,maxValue);
                c=isc.FacetChart._dateSubtract(
                    gradationIncrement,
                    isc.FacetChart._dateFloor(gradationIncrement,ceil),
                    zero);
            }else{
                ceil=Math.max(base,maxValue);
                c=Math.floor(ceil/gradationIncrement);
            }
        }
...
,isc.A._getValueCoord=function isc_FacetChart__getValueCoord(vertical,value,context){
    ...
    if(vertical){
        var top=this._getMainChartAreaTop(),
            ...
            if(transform._isAffine2D){
                ...
                transform={
                    ...
                    transform:function(v0,v1){
                        ...
                        return this._affinePart.transform(w.v0,w.v1);
                    },
...
isc.evalBoundary;isc.B.push(isc.A._getMetricSettingsDefaults=function isc_FacetChart__getMetricSettingsDefaults(){
    var defaults={
    ...
    if(settings==null){
        ...
        for(var i=settings.length;i--;){
            ...
            if(isc.isAn.Array(group)){
                ...
                var clonedGroup=new Array(group.length);
                for(var j=group.length;j--;){
                    clonedGroup[j]=clonedSettings[settings.indexOf(group[j])];
                }
...
,isc.A.drawRadarData=function isc_FacetChart_drawRadarData(rect,maxValue,data){
    ...
    for(var i=0;i<data.length;i++){
        ...
        if(showDataLabels){
            this._drawRadialLabel(center,this.maxRadius,angle,data[i].title,facetValue);
        }
    }
...
,isc.A._drawRadialLabel=function isc_FacetChart__drawRadialLabel(center,radius,angle,label,facetValue){
...
isc.evalBoundary;isc.B.push(isc.A._drawData=function isc_FacetChart__drawData(gradations,logScale,labelDataPairs,rotateLabels,checkDataValueLabels){
    var hasExtraAxes=this._hasExtraAxes(),
        numValueAxes=this._getNumValueAxes(),
        numAxes=1+numValueAxes,
    ...
    for(var i=0;i<dataSeriesLength;++i){
        ...
        var points=drawLines&&new Array(len),
            ...
            badValue=(
                proportional?
                    allZero:
                    (!isc.isA.Number(values[k])||plotValues[k]<axisStartValue));
        for(var j=0;j<len;++j,k+=inc){
            ...
            if(i==0&&!isRadar){
                xs[k]=(discrete?
                    ...
                    this._getValueCoord(false,data[k].title,logScale,context));
            }
            if(badValue){
                if(k==start){
                    leftBadValue=true
...
isc.evalBoundary;isc.B.push(isc.A._drawBars=function isc_FacetChart__drawBars(labelDataPairs,rotateLabels,context,drawBars){
    context=context||this;
            ...
            for(var j=0;j<data[i].series.length;j++){
                ...
                if(proportional){
                    ...
                    if(!badValue){
                        barLength=(
                            context._chartTransform[transformProp](
                                Math.max(axisStartValue,newRunningTotal))-barBase);
                    }
                }
                if(isFirstBar){
                    if(vertical){
...
isc.evalBoundary;isc.B.push(isc.A.drawDataPoint=function isc_FacetChart_drawDataPoint(point,series,size,record,value,context,interactivePoints,autoDraw){
    context=context||this;
...
,isc.A._createDataShape=function isc_FacetChart__createDataShape(shapeType,color,context,bubbleProperties,autoDraw,point,size){
    ...
    if(shapeType==this._$Oval){
        ...
        shape=isc.DrawRect.create({
            lineColor:color,
            fillColor:color
        },context.datPointDefaults,context.dataPointProperties,bubbleProperties,{
            drawPane:this,
            autoDraw:autoDraw,
...
,isc.A._drawPointSizeLegend=function isc_FacetChart__drawPointSizeLegend(){
    ...
    if(useLog){
        var logBase=this.pointSizeLogBase,
            ...
            minMagnitude=this.getMagnitude(minPointSizeValue,logBase,undefined),
            maxMagnitude=this.getMagnitude(maxPointSizeValue,logBase,undefined);
        ...
        var magnitude=this.getMagnitude(
                maxPointSizeValue-minPointSizeValue,10,undefined),
...
,isc.A._drawColorScaleLegend=function isc_FacetChart__drawColorScaleLegend(){
    var metricFacet=this._getMetricFacet(),
        colorScaleMetric=this._getColorScaleMetric();
    ...
    if(logScalePointColor&&this.useColorScaleLogGradations){
        var logGradations=this._validateLogGradations(
            ...
            minMagnitude=this.getMagnitude(minColorScaleValue,logBase,undefined),
            maxMagnitude=this.getMagnitude(maxColorScaleValue,logBase,undefined);
        ...
        var magnitude=this.getMagnitude(
                maxColorScaleValue-minColorScaleValue,10,undefined),
            high=Math.pow(10,magnitude-1),
            low=Math.pow(10,magnitude-2),
            possibleIncrements=[low,2*low,5*low,high,2*high,5*high];
        for(var i=0;i<possibleIncrements.length;++i){
...
isc.evalBoundary;isc.B.push(isc.A.setDataColors=function isc_FacetChart_setDataColors(dataColors,context){
    context=context||this;
...
,isc.A._getMetricColor=function isc_FacetChart__getMetricColor(colorScaleMetricValue){
    ...
    if(useColorStops){
        var colorStops=gradient.colorStops,
            k=isc.FacetChart._binarySearch(
                colorStops,0,colorStops.length-1,"offset",valueRatio,undefined);
        if(k<0){
            k=-(2+k);
        }
        ...
        if(context!=null){
            ...
            if(nearestValue!=null&&this._hoverLabel.contents!==nearestValue.value){
                ...
                if(this._hoverRect.drawingVML){
                    ...
                    this._hoverRect.resizeTo(size.width+8,size.height+8);
...
isc.evalBoundary;isc.B.push(isc.A._hasDiscreteDataValues=function isc_FacetChart__hasDiscreteDataValues(){
    var discrete=(
                        ...
                        for(var j=0,k=start;j<len;++j,++k){
                            ...
                            if(!badValue){
                                ...
                                if(discrete){
                                    ...
                                    var xValueRatio=this.getValueRatio(
                                            data[k].title,lowValue,highValue,logScale,context),
...
isc.B.push(isc.A._createChartRectTransform=function isc_c_FacetChart__createChartRectTransform(left,top,width,height){
    var t=isc.AffineTransform.create({
        ...
        _isAffine2D:true,
        transformX:function(v0){
            return this.transform(v0,0).v0;
        },
        transformY:function(v1){
            return this.transform(0,v1).v1;
        }
    });
    return t;
}
,isc.A._stackTransforms=function isc_c_FacetChart__stackTransforms(tx,ty){
    if(tx._isAffine1D&&ty._isAffine1D){
        ...
        return{
            ...
            transform:function(v0,v1){
                return{
                    v0:this._tx.transform(v0),
                    v1:this._ty.transform(v1)
                };
            },
            transformX:function(v0){
    ...
    if(t._isRound1D||t._isRound2D){
        ...
        return{
            ...
            transform:function(v0,v1){
                ...
                transformed.v0=Math.round(transformed.v0);
                transformed.v1=Math.round(transformed.v1);
                return transformed;
            },
            transformX:function(v0){
