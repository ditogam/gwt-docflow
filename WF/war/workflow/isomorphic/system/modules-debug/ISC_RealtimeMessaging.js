...
,isc.A.subscribe=function isc_c_Messaging_subscribe(channel,callback,subscriptionCallback,data){
    if(!isc.hasOptionalModules("RealtimeMessaging")&&!this.isRemoteDebug){
    ...
    if(fireSubscriptionCallback)this.fireCallback(subscriptionCallback);
    return;
}
...
,isc.A._connectCallback=function isc_c_Messaging__connectCallback(conn,config){
    ...
    for(var key in this._channels){
        ...
        if(channel.subscriptionCallback){
            this.fireCallback(channel.subscriptionCallback);
