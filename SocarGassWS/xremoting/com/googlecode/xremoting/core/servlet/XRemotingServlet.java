...
public abstract class XRemotingServlet extends HttpServlet {
	...
	protected void doInit(ServletConfig config) throws ServletException {
		...
		if (interfacesString == null) {
			throw new ServletException(
					"init-parameter called exposedInterfaces is required");
		}
		...
		restriction = createInvocationRestriction(interfaces
				.toArray(new Class<?>[interfaces.size()]));
	}

	protected InvocationRestriction createInvocationRestriction(
			Class<?>[] interfaces) {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			try {
				invokingHelper.invoke(getTarget(), is, os, serializer, invoker,
						restriction);
			} finally {
