...
/**
 * This is the public test class to try the jproj api. Reads data from file. The
 * input file has three lines of header:<BR>
 * 1) source projection code or option <BR>
 * 2) destination projection code or option<BR>
 * 3) rows of the data<BR>
 * and after the header the data are put in the format: x0 y0 z0 x1 y1 z1 x2 y2
 * z2 ...etc etc
 * 
 * example: srcProj: +proj=latlong +datum=WGS84 destProj: +init=epsg:32632 rows:
 * 2 ...datatriplets
 * 
 */
public class Proj4 implements Proj4Factory {
	...
	/**
	 * constructor used to instatiate a single projection. This is used if the
	 * goal is to simply get information about a projection definition
	 * 
	 * @param proj
	...
	public void transform(ProjectionData dataTP, long point_count,
		...
		/*
		 * System.out.println("Transformed coordinates and values:"); for (int i
		 * = 0; i < dataTP.rows; i++) { System.out.println("x = " + dataTP.x[i]
		 * + " y = " + dataTP.y[i] + " z = " + dataTP.z[i]); }
		 */
	}
}