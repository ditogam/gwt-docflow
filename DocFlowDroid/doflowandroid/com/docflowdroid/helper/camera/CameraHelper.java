		...
		ProcessExecutor.execute(new IProcess() {
			...
			public void execute() throws Exception {
				context.runOnUiThread(new Runnable() {
					@Override
					public void run() {
						// addFilePrivate(context, stream, fileName,
						// cameraResult);
						HttpMultipartPost post = new HttpMultipartPost(context,
								fileName, cameraResult);
						post.execute(new HttpResponse[] {});
					}
				});
		...
		if (isCameraResult(context, requestCode, resultCode)) {
			try {
				...
				if (pickedUri != null) {
					...
					file = new File(context.getCacheDir(), file.getName());
					...
					addFile(context, new ByteArrayInputStream(bt),
							file.getAbsolutePath(), cameraResult);
				}
			} catch (Throwable e) {
