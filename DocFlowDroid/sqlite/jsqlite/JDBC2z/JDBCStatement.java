...
public class JDBCStatement implements java.sql.Statement {
	...
	public void setQueryTimeout(int seconds) throws SQLException {
		...
		if (isClosed()) {
			throw new SQLException(
					"can't set a query timeout on a closed statement");
		} else if (seconds < 0) {
			throw new SQLException(
					"can't set a query timeout of less than 0 seconds");
		} else if (seconds == 0) {
			// An argument of 0 seconds should set an unlimited timeout.
			// However, since this was not
			// done previously, I assume it isn't implemented and use the same
			// implementation.
			conn.timeout = 5000;
		} else {
	...
	public boolean execute(String sql, int autokeys) throws SQLException {
		if (autokeys != Statement.NO_GENERATED_KEYS) {
			throw new SQLFeatureNotSupportedException(
					"autogenerated keys not supported");
		}
		return execute(sql);
	}
	...
	public boolean isClosed() throws SQLException {
		return conn == null; // android-changed: pretty sure this is correct,
								// since it matches what's done in close()
	}
