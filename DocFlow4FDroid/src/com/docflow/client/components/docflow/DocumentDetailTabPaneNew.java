...
public class DocumentDetailTabPaneNew extends VLayout implements
	...
	public static DocumentDetailTabPaneNew documentDetails;
	private int doctypeid;
	private ToolStripButton bEdit;
	private ToolStripButton bStatusEdit;
	...
	public DocumentDetailTabPaneNew() {
		super();
		...
		bStatusEdit = new ToolStripButton("Edit Status",
		...
		bStatusEdit.setDisabled(true);
		toolstrip.addButton(bNew);
		toolstrip.addButton(bEdit);
		toolstrip.addButton(bStatusEdit);
		...
		// wDocument = new WDocument(this);
		HLayout hlPanel = new HLayout();
		/* editTab.setWidth(); */
		wDocument.setPane(hlPanel);
		...
		bStatusEdit.addClickHandler(new ClickHandler() {
		...
		tabset.setTabs(viewTab, statusTab, historyTab, replicaHistoryTab);
		setPermitions();
	}

	private void editDocStatus() {
		bSave.setDisabled(false);
		bNew.setDisabled(false);
		bEdit.setDisabled(true);

		setPermitions();
		this.tabset.selectTab(statusTab);
	...
	private void showDocumentWindow() {
		wDocument.setWidth(Page.getWidth());
		wDocument.setHeight(Page.getHeight());
		if (DocFlow.hasPermition(PermissionNames.DOCFLOW_OPEN_WINDOW_ANIMATION)) {
			Rectangle rect = bNew.getPageRect();
			...
			animateOutline.animateRect(0, 0, Page.getWidth(), Page.getHeight(),
					new AnimationCallback() {
						public void execute(boolean earlyFinish) {
							animateOutline.hide();
							wDocument.setCloseEvent(new CloseClickHandler() {

								@Override
								public void onCloseClick(CloseClickEvent event) {

									animateOutline.setRect(0, 0,
											Page.getWidth(), Page.getHeight());
									animateOutline.show();
									Rectangle targetRect = bNew.getPageRect();
	...
	private String getXml(HashMap<String, Object> data) {
		String ret = "";
	...
	private void setPane(DocumentLong doc) {
		...
		if (lastItem != null) {
			HashMap<String, String> values = getData(cont, false);
			String html = generateHtml(getData(cont, true), null,
					lastItem.getFormitemMap(), doc.getError(), doc.getReplic());
			itemViewer.setContents(html);
			itemViewerStatuse.setContents(html);
			lastItem.setData(values, doc.getCust_id(),null);
	...
	private void setPermitions() {
		...
		setDisabledToButton(cannotaddedit, bStatusEdit);
	}
	...
	private void setVersionId(Object versionid) {
		...
		if (lastItem != null) {
			itemViewerHistory.setContents(generateHtml(
					getData(log.getXml(), true),
					getData(doc.getContent_xml(), true),
	...
	public void settingDataComplete() {
		if (lastItem != null && bSave.getDisabled())
