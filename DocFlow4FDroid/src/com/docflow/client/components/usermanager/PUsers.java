...
public class PUsers extends PSharedClassList {
	...
	protected void setDocTypePermitions() {
		...
		new WDocTypePermitions(user_id, true);
	...
	protected void doAddEditOperation(boolean add, SharedClass cl) {
		...
		// user.setCaption_id(cl.getCaption_id());
	...
	private void setPermitions() {
		...
		if (rec == null)
			return;
		final int user_id = rec.getAttributeAsInt("id");
		DocFlow.docFlowService.getGroupsAndPermitions(true, user_id,
				new AsyncCallback<GroupsAndPermitions>() {
					...
					public void onSuccess(GroupsAndPermitions result) {
						WPermitions.showWindow(true, user_id, result);
		...
		DocFlow.docFlowService.getUserManagerObjects(UMObject.USER,
				new AsyncCallback<HashMap<Integer, ArrayList<UMObject>>>() {
					...
					public void onSuccess(
						...
						setDataUM(result.get(UMObject.USER));
