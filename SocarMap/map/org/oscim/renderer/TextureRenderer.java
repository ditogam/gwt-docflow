...
public final class TextureRenderer {
	...
	public static Layer draw(Layer layer, float scale, float[] projection,
		...
		for (TextureObject to = tl.textures; to != null; to = to.next) {
			...
			// Log.d("...", "draw texture: " + to.id + " " + to.offset + " " +
			// to.vertices);
	...
	static void init() {
		...
		hTextureProjMatrix = GLES20.glGetUniformLocation(mTextureProgram,
				"u_proj");
		hTextureScale = GLES20.glGetUniformLocation(mTextureProgram, "u_scale");
		hTextureScreenScale = GLES20.glGetUniformLocation(mTextureProgram,
				"u_swidth");
		hTextureVertex = GLES20.glGetAttribLocation(mTextureProgram, "vertex");
		hTextureTexCoord = GLES20.glGetAttribLocation(mTextureProgram,
				"tex_coord");

		int bufferSize = MAX_ITEMS * VERTICES_PER_SPRITE * SHORTS_PER_VERTICE
				* (Short.SIZE / 8);
		...
		GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, mIndicesVBO);
		GLES20.glBufferData(GLES20.GL_ELEMENT_ARRAY_BUFFER, len
				* (Short.SIZE / 8), mShortBuffer, GLES20.GL_STATIC_DRAW);
		GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, 0);
	}
}
