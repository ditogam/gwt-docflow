...
public class LineTexRenderer {
	...
	private final static float COORD_SCALE_BY_DIR_SCALE = GLRenderer.COORD_SCALE
			/ LineLayer.DIR_SCALE;
	...
	final static String vertexShader = "" + "precision mediump float;"
			+ "uniform mat4 u_mvp;" + "uniform vec4 u_color;"
			+ "uniform float u_pscale;" + "uniform float u_width;"
			+ "attribute vec4 a_pos0;" + "attribute vec4 a_pos1;"
			+ "attribute vec2 a_len0;" + "attribute vec2 a_len1;"
			+ "attribute float a_flip;" + "varying vec2 v_st;"
			+ "void main() {" + "  vec4 pos;" + "  if (a_flip == 0.0){"
			+ "    vec2 dir = u_width * a_pos0.zw;"
			+ "    pos = vec4(a_pos0.xy + dir, 0.0, 1.0);"
			+ "    v_st = vec2(a_len0.x / u_pscale, 1.0);" + "  } else {"
	...
	public static Layer draw(Layers layers, Layer curLayer, MapPosition pos,
		...
		GLES20.glVertexAttribPointer(hVertexFlip, 1, GLES20.GL_BYTE, false, 0,
				0);
		...
		while (l != null && l.type == Layer.TEXLINE) {
			...
			GLES20.glUniform1f(hPatternScale,
					(GLRenderer.COORD_SCALE * line.stipple) / ps);
			...
			for (int i = 0; i < allIndices; i += maxIndices) {
				...
				GLES20.glVertexAttribPointer(hVertexPosition0, 4,
						GLES20.GL_SHORT, false, STRIDE, add + STRIDE);

				GLES20.glVertexAttribPointer(hVertexLength0, 2,
						GLES20.GL_SHORT, false, STRIDE, add + STRIDE
								+ LEN_OFFSET);

				GLES20.glVertexAttribPointer(hVertexPosition1, 4,
						GLES20.GL_SHORT, false, STRIDE, add);

				GLES20.glVertexAttribPointer(hVertexLength1, 2,
						GLES20.GL_SHORT, false, STRIDE, add + LEN_OFFSET);
			...
			for (int i = 0; i < allIndices; i += maxIndices) {
				...
				GLES20.glVertexAttribPointer(hVertexPosition0, 4,
						GLES20.GL_SHORT, false, STRIDE, add + 2 * STRIDE);
	...
	public static void init() {
		...
		ByteBuffer buf = ByteBuffer.allocateDirect(maxIndices * 2).order(
	 ...
	 * " uniform float u_scale;" + " uniform vec4 u_color;" +
	 * " uniform vec4 u_bgcolor;" + " varying vec2 v_st;" + " void main() {" +
	 * "  float len = texture2D(tex, v_st).a;" + "  float tex_w = abs(v_st.t);"
	 ...
	 * "#extension GL_OES_standard_derivatives : enable\n" +
	 * " precision mediump float;" + " uniform sampler2D tex;" +
	 * " uniform vec4 u_color;" + " uniform vec4 u_bgcolor;" +
	 * " uniform float u_pwidth;" + " varying vec2 v_st;" + " void main() {" +
	 * "  float dist = texture2D(tex, v_st).a;" + "  float tex_w = abs(v_st.t);"
	 * + "  vec2 st_width = fwidth(v_st);" +
	 * "  float fuzz = max(st_width.s, st_width.t);" +
	 * "  float line_w    = (1.0 - smoothstep(1.0 - fuzz, 1.0, tex_w));" +
