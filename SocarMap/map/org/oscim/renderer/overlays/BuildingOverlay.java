...
public class BuildingOverlay extends RenderOverlay {
	...
	private void addOutline(float[] points, float height) {
		...
		for (int j = 0; j < 2; j++) {
			...
			for (int k = j * 2; k < len; k += 4) {
				...
				indices[cnt++] = s3;
				indices[cnt++] = s2;
				// System.out.println("indice:" + k + "\t" + s0 + "," + s1 + ","
				// + s2);
				// System.out.println("indice:" + k + "\t" + s1 + "," + s3 + ","
				// + s2);
			}
	...
	public synchronized void render(MapPosition pos, Matrices m) {
		...
		GLState.enableVertexArrays(hBuildingVertexPosition,
				hBuildingLightPosition);
	...
	public synchronized void update(MapPosition curPos,
		...
		hBuildingColor = GLES20
				.glGetUniformLocation(buildingProgram, "u_color");
		hBuildingMode = GLES20.glGetUniformLocation(buildingProgram, "u_mode");
		hBuildingVertexPosition = GLES20.glGetAttribLocation(buildingProgram,
				"a_position");
		hBuildingLightPosition = GLES20.glGetAttribLocation(buildingProgram,
				"a_light");
		...
		float[] points = { -200, -200, 200, -200, 200, 200, -200, 200, -300, 0 };
		addOutline(points, height);

		float[] points2 = { 300, -300, 500, -300, 600, 100, 300, 100,
		// 350, 0
		};
		...
		ByteBuffer buf = ByteBuffer.allocateDirect(bufferSize).order(
		...
		for (int i = 0; i < 3; i++) {
			for (VertexPoolItem vi = mIndices[i]; vi != null; vi = vi.next) {
				System.out.println("put indices: " + vi.used + " "
						+ mNumIndices);
		...
		sbuf.flip();
		GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, mIndicesBufferID);
		GLES20.glBufferData(GLES20.GL_ELEMENT_ARRAY_BUFFER, mNumIndices * 2,
		...
		sbuf.flip();
		GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, mVertexBufferID);
		GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, mNumVertices * 4 * 2, sbuf,
