...
public class MapView extends RelativeLayout {
	...
	public void setColors(int stroke_color, int start_color, int end_color) {
		mGLView.setColors(stroke_color, start_color, end_color);
	}
	...
	public MapView(Context context, AttributeSet attributeSet,
			int stroke_color, int start_color, int end_color) {
		...
		for (int i = 0; i < mNumMapWorkers; i++) {
			...
			mMapWorkers[i] = new MapWorker(i, mJobQueue, tileGenerator,
					mTileManager);
			mMapWorkers[i].start();
		}
		...
		// mOverlayManager.add(new GenericOverlay(this, new
		// TestLineOverlay(this)));
	...
	public void createCompass(int compass_id) {

	}

	public void createLocationHandler(int button_id) {
		if (locationHandler == null)
			locationHandler = new LocationHandler(this, button_id);
	}
	...
	public Float getCompassAngle() {
		if (!getCompassEnabled())
			return null;
		return mCompass.getAngle();
	}

	public float getRealAngle() {
		return mCompass.getRealAngle();
	}
	...
	public MapPosition getMapFileCenter() {
		...
		if (mapInfo.startZoomLevel != null)
			return new MapPosition(startPos,
					(mapInfo.startZoomLevel).byteValue(), 1);
	...
	public boolean setMapDatabase(MapOptions options) {
		...
		for (int i = 0; i < mNumMapWorkers; i++) {
			...
			OpenResult result = mapDatabase.open(options, getContext());
	...
	public void setRenderTheme(String renderThemePath)
		...
		if (renderThemePath == null) {
			throw new IllegalArgumentException(
					"render theme path must not be null");
		}
	...
	public void setMapCenter(MapPosition mapPosition) {
		Log.d(TAG, "setMapCenter " + " lat: " + mapPosition.lat + " lon: "
				+ mapPosition.lon);
