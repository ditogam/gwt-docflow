			...
			if (ELEMENT_NAME_RENDER_THEME.equals(localName)) {
				...
				if ((style = attributes.getValue("from")) != null) {
					...
					if (ri instanceof Line) {
						Line line = Line.create((Line) ri, localName,
								attributes, 0, false);
					...
					Line line = Line.create(null, localName, attributes, 0,
							false);
					tmpStyleHash.put("l" + line.style, line);
				}
			}
				...
				Line line = Line.create(null, localName, attributes, mLevel++,
						true);
				...
				checkState(localName, Element.RENDERING_INSTRUCTION);
				LineSymbol lineSymbol = LineSymbol
						.create(localName, attributes);
				...
				if (style != null) {
					...
					if (area != null)
						mCurrentRule.addRenderingInstruction(new AreaLevel(
								area, mLevel++));
					else
