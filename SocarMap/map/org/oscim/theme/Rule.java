...
abstract class Rule {
	...
	static Rule create(String elementName, Attributes attributes,
		...
		return createRule(ruleStack, element, keys, values, closed, zoomMin,
				zoomMax);
	}

	private static Rule createRule(Stack<Rule> ruleStack, int element,
			String keys, String values, int closed, byte zoomMin, byte zoomMax) {
		...
		if (valueList.remove(STRING_NEGATION)) {
			AttributeMatcher attributeMatcher = new NegativeMatcher(keyList,
					valueList, false);
		...
		return new PositiveRule(element, closed, zoomMin, zoomMax, keyMatcher,
				valueMatcher);
	}
	...
	private static void validate(String elementName, String keys,
	...
	void matchNode(IRenderCallback renderCallback, Tag[] tags, byte zoomLevel,
		...
		if ((mElement != Element.WAY) && mZoomMin <= zoomLevel
				&& mZoomMax >= zoomLevel && matchesNode(tags)) {
	...
	void matchWay(IRenderCallback renderCallback, Tag[] tags, byte zoomLevel,
		...
		if ((mElement != Element.NODE) && mZoomMin <= zoomLevel
				&& mZoomMax >= zoomLevel
			...
			// check subrules
			for (int i = 0, n = mSubRuleArray.length; i < n; i++)
				mSubRuleArray[i].matchWay(renderCallback, tags, zoomLevel,
						closed, matchingList);
	...
	void onComplete() {
		...
		mRenderInstructionArray = new RenderInstruction[mRenderInstructions
				.size()];
