...
public class RenderTheme {
	...
	private static void validate(String elementName, Integer version,
		...
		if (version == null) {
			throw new IllegalArgumentException(
					"missing attribute version for element:" + elementName);
		} else if (version.intValue() != RENDER_THEME_VERSION) {
			throw new IllegalArgumentException("invalid render theme version:"
					+ version);
		} else if (baseStrokeWidth < 0) {
			throw new IllegalArgumentException(
					"base-stroke-width must not be negative: "
							+ baseStrokeWidth);
		} else if (baseTextSize < 0) {
			throw new IllegalArgumentException(
					"base-text-size must not be negative: " + baseTextSize);
		}
	}
	...
	private ArrayList<RenderInstruction> mWayInstructionList = new ArrayList<RenderInstruction>(
			4);

	private ArrayList<RenderInstruction> mAreaInstructionList = new ArrayList<RenderInstruction>(
			4);
	private ArrayList<RenderInstruction> mNodeInstructionList = new ArrayList<RenderInstruction>(
			4);
	...
	public RenderInstruction[] matchNode(IRenderCallback renderCallback,
		...
		synchronized (mNodesCache) {
			...
			if (ri == null) {
				...
				matches.clear();
				for (int i = 0, n = mRulesList.size(); i < n; ++i)
					mRulesList.get(i).matchNode(renderCallback, tags,
							zoomLevel, matches);
	...
	public RenderInstruction[] matchWay(IRenderCallback renderCallback,
		...
		synchronized (matchingCache) {
			...
			if (prevInstructions == null
					|| (prevInstructions.zoom & zoomMask) == 0) {
				// previous instructions zoom does not match
				cacheKey.set(tags, null);
			} else {
				// compare if tags match previous instructions
			...
			if (ri == null) {
				...
				for (int i = 0, n = mRulesList.size(); i < n; ++i)
					mRulesList.get(i).matchWay(renderCallback, tags, zoomLevel,
							c, matches);
