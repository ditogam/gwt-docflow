...
public final class Text extends RenderInstruction {
	private static Typeface typefaceNormal = Typeface.create(
			FontFamily.DEFAULT.toTypeface(), FontStyle.NORMAL.toInt());

	private static Typeface typefaceBold = Typeface.create(
			FontFamily.DEFAULT.toTypeface(), FontStyle.BOLD.toInt());
	 ...
	 */
	public static Text create(String elementName, Attributes attributes,
			boolean caption) {
		String textKey = null;
		...
		for (int i = 0; i < attributes.getLength(); ++i) {
			...
			if ("name".equals(name))
				...
				fontFamily = FontFamily.valueOf(value
				...
				RenderThemeHandler.logUnknownAttribute(elementName, name,
						value, i);
			}
		}
		...
		if (typeface == null)
			typeface = Typeface.create(fontFamily.toTypeface(),
					fontStyle.toInt());

		return new Text(style, textKey, typeface, fontSize, fill, stroke,
				strokeWidth, dy, caption, priority);
	}

	public static Text createText(float fontSize, float strokeWidth, int fill,
			int outline, boolean billboard) {

		return new Text("", "", typefaceNormal, fontSize, fill, outline,
				strokeWidth, 0, billboard, Integer.MAX_VALUE);
	}

	private static void validate(String elementName, String textKey,
			float fontSize, float strokeWidth) {
		if (textKey == null) {
			throw new IllegalArgumentException(
					"missing attribute k for element: " + elementName);
		} else if (fontSize < 0) {
			throw new IllegalArgumentException(
					"font-size must not be negative: " + fontSize);
		} else if (strokeWidth < 0) {
			throw new IllegalArgumentException(
					"stroke-width must not be negative: " + strokeWidth);
		}
	}
	...
	private Text(String style, String textKey, Typeface typeface,
			float fontSize, int fill, int outline, float strokeWidth, float dy,
			boolean caption, int priority) {
