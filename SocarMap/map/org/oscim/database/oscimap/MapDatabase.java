...
public class MapDatabase implements IMapDatabase {
	...
	private String decodeString() throws IOException {
		...
		final String result = new String(lwHttp.buffer, lwHttp.bufferPos, size,
				"UTF-8");
	...
	private boolean decodeTileElement(int type) throws IOException {
		...
		if (fail || tags == null || indexCnt == 0) {
			Log.d(TAG, mTile + " failed reading way: bytes:" + bytes
					+ " index:"
					+ (index != null ? Arrays.toString(index) : "null")
					+ " tag:"
		...
		if (type == TAG_TILE_LINE)
			...
			mMapGenerator.renderPointOfInterest((byte) layer, tags, coords[1],
					coords[0]);
		}
	...
	private int decodeWayCoordinates(boolean skip, int nodes)
		...
		while (pos < end) {
			if (buf[pos] >= 0) {
				...
				result = (buf[pos++] & 0x7f) | buf[pos++] << 7;
			} else if (buf[pos + 2] >= 0) {
				result = (buf[pos++] & 0x7f) | (buf[pos++] & 0x7f) << 7
						| (buf[pos++]) << 14;
			} else if (buf[pos + 3] >= 0) {
				result = (buf[pos++] & 0x7f) | (buf[pos++] & 0x7f) << 7
						| (buf[pos++] & 0x7f) << 14 | (buf[pos++]) << 21;
			} else {
				result = (buf[pos] & 0x7f) | (buf[pos + 1] & 0x7f) << 7
						| (buf[pos + 2] & 0x7f) << 14
						| (buf[pos + 3] & 0x7f) << 21 | (buf[pos + 4]) << 28;
	...
	public QueryResult executeQuery(JobTile tile,
		...
		try {
			...
			if (lwHttp.sendRequest(tile)
					&& (mContentLenth = lwHttp.readHeader()) >= 0) {
