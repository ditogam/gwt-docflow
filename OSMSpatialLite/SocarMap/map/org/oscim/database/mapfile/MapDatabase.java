...
public class MapDatabase implements IMapDatabase {
	 ...
	 */
	public static final int POI_FEATURE_NAME = 0x80;
	...
	private int decodeWayNodesDoubleDelta(int length) {
		...
		for (int pos = 2; pos < length; pos += 2) {
			...
			if (dLon > minLon || dLon < -minLon || dLat > minLat
					|| dLat < -minLat || (pos == length - 2)) {
	...
	public OpenResult open(MapOptions options, Context context) {
		try {
			...
			Log.d(TAG,
					"open instance " + instances + " " + file.getAbsolutePath());
			if (instances++ > 0) {
			...
			Log.d(TAG, "File version: "
					+ sMapFileHeader.getMapFileInfo().fileVersion);
			return OpenResult.SUCCESS;
		} catch (IOException e) {
			Log.e(TAG, e.getMessage());
	...
	private void processBlocks(IMapDatabaseCallback mapDatabaseCallback,
		...
		for (long row = queryParameters.fromBlockY; row <= queryParameters.toBlockY; ++row) {
			for (long column = queryParameters.fromBlockX; column <= queryParameters.toBlockX; ++column) {
				...
				long currentBlockIndexEntry = sDatabaseIndexCache
				...
				if (currentBlockPointer < 1
					...
					Log.w(TAG, "invalid current block pointer: "
							+ currentBlockPointer);
				...
				if (blockNumber + 1 == subFileParameter.numberOfBlocks) {
					...
					if (nextBlockPointer < 1
							|| nextBlockPointer > subFileParameter.subFileSize) {
						Log.w(TAG, "invalid next block pointer: "
								+ nextBlockPointer);
						Log.w(TAG, "sub-file size: "
				...
				if (currentBlockSize < 0) {
					...
					// block
					Log.w(TAG, "current block size too large: "
							+ currentBlockSize);
					continue;
				...
				mInputFile.seek(subFileParameter.startAddress
						+ currentBlockPointer);

				// read the current block into the buffer
				if (!mReadBuffer.readFromFile(currentBlockSize)) {
					// skip the current block
					Log.w(TAG, "reading current block has failed: "
							+ currentBlockSize);
					return;
				}
	 ...
	 *         otherwise.
	 */
	private boolean processPOIs(IMapDatabaseCallback mapDatabaseCallback,
			int numberOfPois) {
		...
		for (int elementCounter = numberOfPois; elementCounter != 0; --elementCounter) {
			...
			mapDatabaseCallback.renderPointOfInterest(layer, curTags, latitude,
					longitude);
	...
	private short[] processWayDataBlock(boolean doubleDeltaEncoding) {
		...
		for (int coordinateBlock = 0; coordinateBlock < numBlocks; ++coordinateBlock) {
			...
			if (numWayNodes < 2
					|| numWayNodes > MAXIMUM_WAY_NODES_SEQUENCE_LENGTH) {
	...
	private boolean processWays(QueryParameters queryParameters,
			IMapDatabaseCallback mapDatabaseCallback, int numberOfWays) {
		...
		for (int elementCounter = numberOfWays; elementCounter != 0; --elementCounter) {
			...
			if (queryParameters.useTileBitmask) {
				elementCounter = mReadBuffer.skipWays(
						queryParameters.queryTileBitmask, elementCounter);
			...
			if (sMapExperimental) {
				...
				if ((featureByte & WAY_FEATURE_NAME) != 0) {
					...
					String str = mReadBuffer
							.readUTF8EncodedStringAt(stringOffset + textPos);
			...
			if ((featureByte & WAY_FEATURE_DATA_BLOCKS_BYTE) != 0) {
				...
				if (wayDataBlocks < 1) {
					Log.w(TAG, "invalid number of way data blocks: "
							+ wayDataBlocks);
					logDebugSignatures();
					return false;
				}
			...
			for (int wayDataBlock = 0; wayDataBlock < wayDataBlocks; ++wayDataBlock) {
				...
				mapDatabaseCallback.renderWay(layer, curTags, mWayNodes,
						wayLengths, closed, 0);
			}
		}
	...
	private boolean projectToTile(float[] coords, short[] indices) {
		...
		for (int pos = 0, outPos = 0, i = 0, m = indices.length; i < m; i++) {
			...
			for (int end = pos + len; pos < end; pos += 2) {
				...
				lat = (float) (Tile.TILE_SIZE - (Math.log((1.0 + sinLat)
						/ (1.0 - sinLat))
						* divy + dy));
			...
			if (coords[first] == coords[outPos - 2]
					&& coords[first + 1] == coords[outPos - 1]) {
