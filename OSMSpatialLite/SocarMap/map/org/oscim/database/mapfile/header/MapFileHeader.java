...
public class MapFileHeader {
	 ...
	 */
	public OpenResult readHeader(ReadBuffer readBuffer, long fileSize)
			throws IOException {
		...
		openResult = RequiredFields.readFileVersion(readBuffer,
				mapFileInfoBuilder);
		if (!openResult.isSuccess()) {
			return openResult;
		}

		openResult = RequiredFields.readFileSize(readBuffer, fileSize,
				mapFileInfoBuilder);
		if (!openResult.isSuccess()) {
			return openResult;
		}
		...
		openResult = RequiredFields.readBoundingBox(readBuffer,
				mapFileInfoBuilder);
		if (!openResult.isSuccess()) {
			return openResult;
		}

		openResult = RequiredFields.readTilePixelSize(readBuffer,
				mapFileInfoBuilder);
		if (!openResult.isSuccess()) {
			return openResult;
		}

		openResult = RequiredFields.readProjectionName(readBuffer,
				mapFileInfoBuilder);
		if (!openResult.isSuccess()) {
			return openResult;
		}

		openResult = OptionalFields.readOptionalFields(readBuffer,
				mapFileInfoBuilder);
		if (!openResult.isSuccess()) {
			return openResult;
		}
		...
		openResult = readSubFileParameters(readBuffer, fileSize,
				mapFileInfoBuilder);
		if (!openResult.isSuccess()) {
			return openResult;
		}
	...
	private OpenResult readSubFileParameters(ReadBuffer readBuffer,
			long fileSize, MapFileInfoBuilder mapFileInfoBuilder) {
		// get and check the number of sub-files (1 byte)
		byte numberOfSubFiles = readBuffer.readByte();
		if (numberOfSubFiles < 1) {
			return new OpenResult("invalid number of sub-files: "
					+ numberOfSubFiles);
		}
		...
		for (byte currentSubFile = 0; currentSubFile < numberOfSubFiles; ++currentSubFile) {
			...
			if (baseZoomLevel < 0 || baseZoomLevel > BASE_ZOOM_LEVEL_MAX) {
				return new OpenResult("invalid base zooom level: "
						+ baseZoomLevel);
			}
			...
			if (zoomLevelMin < 0 || zoomLevelMin > 22) {
				return new OpenResult("invalid minimum zoom level: "
						+ zoomLevelMin);
			}
			...
			if (zoomLevelMax < 0 || zoomLevelMax > 22) {
				return new OpenResult("invalid maximum zoom level: "
						+ zoomLevelMax);
			}
			...
			if (zoomLevelMin > zoomLevelMax) {
				return new OpenResult("invalid zoom level range: "
						+ zoomLevelMin + SPACE + zoomLevelMax);
			}
			...
			tempSubFileParameters[currentSubFile] = subFileParameterBuilder
					.build();
