...
final class RequiredFields {
	...
	static OpenResult readBoundingBox(ReadBuffer readBuffer,
		...
		if (minLatitude > maxLatitude) {
			return new OpenResult("invalid latitude range: " + minLatitude
					+ SPACE + maxLatitude);
		} else if (minLongitude > maxLongitude) {
			return new OpenResult("invalid longitude range: " + minLongitude
					+ SPACE + maxLongitude);
		}

		mapFileInfoBuilder.boundingBox = new BoundingBox(minLatitude,
	...
	static OpenResult readFileVersion(ReadBuffer readBuffer,
			MapFileInfoBuilder mapFileInfoBuilder) {
	...
	static OpenResult readMapDate(ReadBuffer readBuffer,
			MapFileInfoBuilder mapFileInfoBuilder) {
	...
	static OpenResult readPoiTags(ReadBuffer readBuffer,
		...
		for (int currentTagId = 0; currentTagId < numberOfPoiTags; ++currentTagId) {
			...
			if (tag == null) {
				return new OpenResult("POI tag must not be null: "
						+ currentTagId);
			}
	...
	static OpenResult readRemainingHeader(ReadBuffer readBuffer)
			throws IOException {
		...
		if (remainingHeaderSize < HEADER_SIZE_MIN
			...
			return new OpenResult("invalid remaining header size: "
					+ remainingHeaderSize);
		}

		// read the header data into the buffer
		if (!readBuffer.readFromFile(remainingHeaderSize)) {
			return new OpenResult("reading header data has failed: "
					+ remainingHeaderSize);
		}
		return OpenResult.SUCCESS;
	}

	static OpenResult readTilePixelSize(ReadBuffer readBuffer,
			MapFileInfoBuilder mapFileInfoBuilder) {
	...
	static OpenResult readWayTags(ReadBuffer readBuffer,
		...
		for (int currentTagId = 0; currentTagId < numberOfWayTags; ++currentTagId) {
			...
			String tag = readBuffer.readUTF8EncodedString();
			if (tag == null) {
				return new OpenResult("way tag must not be null: "
						+ currentTagId);
			}
