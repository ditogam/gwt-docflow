...
public class ReadBuffer {
	...
	private static final Logger LOG = Logger.getLogger(ReadBuffer.class
			.getName());
	...
	public long readLong() {
		...
		return (data[pos] & 0xffL) << 56 | (data[pos + 1] & 0xffL) << 48
				| (data[pos + 2] & 0xffL) << 40 | (data[pos + 3] & 0xffL) << 32
				| (data[pos + 4] & 0xffL) << 24 | (data[pos + 5] & 0xffL) << 16
				| (data[pos + 6] & 0xffL) << 8 | (data[pos + 7] & 0xffL);
	...
	public int readShort() {
		mBufferPosition += 2;
		return mBufferData[mBufferPosition - 2] << 8
				| (mBufferData[mBufferPosition - 1] & 0xff);
	}
	...
	public int readSignedInt() {
		...
		if ((data[pos + 1] & 0x80) == 0) {
			...
			return (((data[pos] & 0x7f) | (data[pos + 1] & 0x3f) << 7) ^ -flag)
					+ flag;
		...
		if ((data[pos + 2] & 0x80) == 0) {
			...
			return (((data[pos] & 0x7f) | (data[pos + 1] & 0x7f) << 7 | (data[pos + 2] & 0x3f) << 14) ^ -flag)
					+ flag;
		...
		if ((data[pos + 3] & 0x80) == 0) {
			...
			return (((data[pos] & 0x7f) | ((data[pos + 1] & 0x7f) << 7)
					| ((data[pos + 2] & 0x7f) << 14) | ((data[pos + 3] & 0x3f) << 21)) ^ -flag)
					+ flag;
		}
		...
		return ((((data[pos] & 0x7f) | (data[pos + 1] & 0x7f) << 7
				| (data[pos + 2] & 0x7f) << 14 | (data[pos + 3] & 0x7f) << 21 | (data[pos + 4] & 0x3f) << 28)) ^ -flag)
				+ flag;
	...
	public void readSignedInt(int[] values, int length) {
		...
		for (int i = 0; i < length; i++) {
			...
			if ((data[pos] & 0x80) == 0) {
				...
				values[i] = (((data[pos] & 0x7f) | ((data[pos + 1] & 0x3f) << 7)) ^ -flag)
						+ flag;
				pos += 2;
				...
				values[i] = (((data[pos] & 0x7f)
						| ((data[pos + 1] & 0x7f) << 7) | ((data[pos + 2] & 0x3f) << 14)) ^ -flag)
						+ flag;
				pos += 3;
				...
				values[i] = ((((data[pos] & 0x7f)
						| ((data[pos + 1] & 0x7f) << 7)
						| ((data[pos + 2] & 0x7f) << 14)
						| ((data[pos + 3] & 0x7f) << 21) | ((data[pos + 4] & 0x3f) << 28))) ^ -flag)
						+ flag;
	...
	public String readUTF8EncodedString(int stringLength) {
		if (stringLength > 0
				&& mBufferPosition + stringLength <= mBufferData.length) {
			mBufferPosition += stringLength;
			try {
				return new String(mBufferData, mBufferPosition - stringLength,
						stringLength, CHARSET_UTF8);
