...
class IndexCache {
	...
	IndexCache(RandomAccessFile randomAccessFile, int capacity) {
		...
		this.map = Collections
				.synchronizedMap(new LRUCache<IndexCacheEntryKey, byte[]>(
						capacity));
	}
	 ...
	 */
	synchronized long getIndexEntry(SubFileParameter subFileParameter,
			long blockNumber) {
		try {
			...
			IndexCacheEntryKey indexCacheEntryKey = new IndexCacheEntryKey(
					subFileParameter, indexBlockNumber);

			// check for cached index block
			byte[] indexBlock = this.map.get(indexCacheEntryKey);
			if (indexBlock == null) {
				// cache miss, seek to the correct index block in the file and
				// read it
				long indexBlockPosition = subFileParameter.indexStartAddress
						+ indexBlockNumber * SIZE_OF_INDEX_BLOCK;

				int remainingIndexSize = (int) (subFileParameter.indexEndAddress - indexBlockPosition);
				int indexBlockSize = Math.min(SIZE_OF_INDEX_BLOCK,
						remainingIndexSize);
			...
			return Deserializer.getFiveBytesLong(indexBlock,
					addressInIndexBlock);
		} catch (IOException e) {
