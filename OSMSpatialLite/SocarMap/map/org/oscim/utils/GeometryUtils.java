...
public final class GeometryUtils {
	public static float area(float ax, float ay, float bx, float by, float cx,
			float cy) {
		float area = ((ax - cx) * (by - cy) - (bx - cx) * (ay - cy)) * 0.5f;
		return area < 0 ? -area : area;
	}

	public static float areaSigned(float ax, float ay, float bx, float by,
			float cx, float cy) {
	...
	public static byte calcLinesIntersect(double ax1, double ay1, double ax2,
	...
	static float[] calculateCenterOfBoundingBox(float[] coordinates) {
		...
		return new float[] { (longitudeMin + longitudeMax) / 2,
				(latitudeMax + latitudeMin) / 2 };
	}

	/*
	 * from World Wind Android: Copyright (C) 2012 United States Government as
	 * represented by the Administrator of the National Aeronautics and Space
	 * Administration. All Rights Reserved. . given the current and previous
	 * locations of two points, compute the angle of the rotation they trace out
	 */
	public static double computeRotationAngle(float x, float y, float x2,
			float y2, float xPrev, float yPrev, float xPrev2, float yPrev2) {
		...
		// compute lengths of sides of triangle created by pt1, pt1Prev and the
		// intersection pt
		double BC = Math
				.sqrt(Math.pow(x - isectX, 2) + Math.pow(y - isectY, 2));
		double AC = Math.sqrt(Math.pow(xPrev - isectX, 2)
				+ Math.pow(yPrev - isectY, 2));
		...
		// if one finger stayed fixed, may have degenerate triangle, so use
		// other triangle instead
		if (BC == 0 || AC == 0 || AB == 0) {
			BC = Math.sqrt(Math.pow(x2 - isectX, 2) + Math.pow(y2 - isectY, 2));
			AC = Math.sqrt(Math.pow(xPrev2 - isectX, 2)
					+ Math.pow(yPrev2 - isectY, 2));
	...
	static boolean doesIntersect(double l1x1, double l1y1, double l1x2,
	...
	public static boolean lineIntersect(int x1, int y1, int x2, int y2, int x3,
	...
	public static byte linesIntersect(double x1, double y1, double x2,
	...
	public static boolean pointInPoly(int nvert, float[] vert, float testx,
			float testy) {
		int i, j;
	...
	public static float pointLineDistance(float x1, float y1, float x2,
			float y2, float x3, float y3) {
		// taken from kartograph/simplify/douglas_peucker.py:
		// the perpendicular distance from a point (x3,y3) to the line from
		// (x1,y1) to (x2,y2)
