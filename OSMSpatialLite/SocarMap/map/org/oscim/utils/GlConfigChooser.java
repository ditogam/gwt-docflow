...
public class GlConfigChooser implements GLSurfaceView.EGLConfigChooser {
	...
	public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display) {
		...
		int[] configSpec = { EGL10.EGL_RED_SIZE, 5, EGL10.EGL_GREEN_SIZE, 6,
				EGL10.EGL_BLUE_SIZE, 5, EGL10.EGL_ALPHA_SIZE, 8,
		...
		if (numConfigs <= 0) {
			...
			configSpec = new int[] {
					...
					EGL10.EGL_DEPTH_SIZE, 16, EGL10.EGL_RENDERABLE_TYPE,
					4 /* EGL_OPENGL_ES2_BIT */, EGL10.EGL_STENCIL_SIZE, 8,
					EGL10.EGL_NONE };
		...
		// 0) == 8
		// // &&
		// // findConfigAttrib(egl, display, configs[i], EGL10.EGL_ALPHA_SIZE,
		// 0) == 0) {
		// // index = i;
		// // break;
		// // }
		// // else
		// if (findConfigAttrib(egl, display, configs[i], EGL10.EGL_RED_SIZE, 0)
		// == 8
		// &&
		// findConfigAttrib(egl, display, configs[i], EGL10.EGL_ALPHA_SIZE, 0)
		// == 0
		// &&
	...
	private String printConfig(EGL10 egl, EGLDisplay display, EGLConfig config) {
		 ...
		 * EGL_SLOW_CONFIG 0x3050 #define EGL_NON_CONFORMANT_CONFIG 0x3051
		 */

		return String.format("EGLConfig rgba=%d%d%d%d depth=%d stencil=%d",
				Integer.valueOf(r), Integer.valueOf(g), Integer.valueOf(b),
				Integer.valueOf(a), Integer.valueOf(d), Integer.valueOf(s))
				+ " native="
				+ findConfigAttrib(egl, display, config,
						EGL10.EGL_NATIVE_RENDERABLE, 0)
				+ " buffer="
				+ findConfigAttrib(egl, display, config, EGL10.EGL_BUFFER_SIZE,
						0)
				+ String.format(" caveat=0x%04x", Integer
						.valueOf(findConfigAttrib(egl, display, config,
