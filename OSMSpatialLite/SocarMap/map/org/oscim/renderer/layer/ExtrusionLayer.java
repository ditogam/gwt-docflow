...
/**
 * @author Hannes Janetzek FIXME check if polygon has self intersections or
 *         0/180 degree angles! or bad things might happen in Triangle
 */
public class ExtrusionLayer extends Layer {
	...
	public static synchronized int triangulate(float[] points, int ppos,
			int plen, short[] index, int ipos, int rings, int vertexOffset,
			VertexPoolItem item) {

		if (!initialized) {
			// FIXME also cleanup on shutdown!
			fBuf = ByteBuffer.allocateDirect(1200 * 4)
					.order(ByteOrder.nativeOrder()).asFloatBuffer();

			sBuf = ByteBuffer.allocateDirect(1800 * 2)
	...
	private void addRoof(int startVertex, short[] index, int ipos,
			float[] points, int ppos) {
		int len = 0;
		int rings = 0;
	...
	public void compile(ShortBuffer sbuf) {
		...
		GLES20.glBufferData(GLES20.GL_ELEMENT_ARRAY_BUFFER, mIndiceBO.size,
				sbuf, GLES20.GL_DYNAMIC_DRAW);
		...
		GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, mVertexBO.size, sbuf,
