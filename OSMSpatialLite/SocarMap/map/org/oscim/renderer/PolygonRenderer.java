...
public final class PolygonRenderer {
	...
	/**
	 * draw polygon layers (unil layer.next is not polygon layer) using stencil
	 * buffer method
	 * 
	 * @param pos
	 *            used to fade layers accorind to 'fade' in layer.area.
	 * @param layer
	 ...
	 * @return next layer
	 */
	public static Layer draw(MapPosition pos, Layer layer, float[] matrix,
			boolean first, boolean clip) {

		GLState.test(false, true);

		GLState.useProgram(polygonProgram);
		GLState.enableVertexArrays(hPolygonVertexPosition, -1);
		glVertexAttribPointer(hPolygonVertexPosition, 2, GL_SHORT, false, 0,
	...
	static void drawOver(float[] matrix) {
		if (GLState.useProgram(polygonProgram)) {
			...
			glVertexAttribPointer(hPolygonVertexPosition, 2, GL_SHORT, false,
					0, POLYGON_VERTICES_DATA_POS_OFFSET);

			glUniformMatrix4fv(hPolygonMatrix, 1, false, matrix, 0);
		}

		/*
		 * clear stencilbuffer (tile region) by drawing a quad with func
		 * 'always' and op 'zero'
		 */
	 ...
	 * 
	 * @param first
	 *            in the first run the clip region is set based on depth buffer
	 *            and depth buffer is updated
	 */
	...
	private static void fillPolygons(int start, int end, int zoom, float scale) {
		...
		for (int c = start; c < end; c++) {
			...
			if (a.fade >= zoom) {
				...
				if (f < 1) {
					GlUtils.setColor(hPolygonColor, a.color, f * a.color[3]);
				} else {
				...
				if (a.blend == zoom)
					GlUtils.setBlendColors(hPolygonColor, a.color,
