...
public class ExtrusionOverlay extends RenderOverlay {
	...
	private final float[] mColor = {
			...
			_a * ((_r - _s) / 255 + 0.01f), _a * ((_g - _s) / 255 + 0.01f),
			_a * ((_b - _s) / 255), _a, _a * ((_r - _s) / 255),
			_a * ((_g - _s) / 255), _a * ((_b - _s) / 255), _a,
			// roof outline
			(_r - _o) / 255, (_g - _o) / 255, (_b - _o) / 255,
			// 1, 0, 0,
			1.0f, };
	...
	final static String extrusionFragmentShader = ""
			+ "precision mediump float;" + "varying vec4 color;"
			+ "void main() {" + "  gl_FragColor = color;" + "}";
	final static String extrusionFragmentShaderZ = ""
			+ "precision highp float;" + "uniform vec4 u_color;"
			+ "varying float z;" + "void main() {" + "if (z < 0.0)"
			+ "  gl_FragColor = vec4(z * -1.0, 0.0, 0.0, 1.0);" + "else"
	...
	public void render(MapPosition pos, Matrices m) {
		// TODO one could render in one pass to texture and then draw the
		// texture
		...
		if (debug) {
			...
			for (int i = 0; i < mTileCnt; i++) {
				...
				GLES20.glDrawElements(GLES20.GL_TRIANGLES, (el.mIndiceCnt[0]
						+ el.mIndiceCnt[1] + el.mIndiceCnt[2]),
						GLES20.GL_UNSIGNED_SHORT, 0);

				GLES20.glDrawElements(GLES20.GL_LINES, el.mIndiceCnt[3],
						GLES20.GL_UNSIGNED_SHORT, (el.mIndiceCnt[0]
								+ el.mIndiceCnt[1] + el.mIndiceCnt[2]) * 2);
	...
	public void update(MapPosition curPos, boolean positionChanged,
		...
		if (!initialized) {
			...
			for (int i = 1; i < 2; i++) {
				...
				shaderProgram[i] = GlUtils.createProgram(
						extrusionVertexShader[i], extrusionFragmentShader);
				if (shaderProgram[i] == 0) {
					Log.e(TAG, "Could not create extrusion shader program. "
							+ i);
					return;
				}
				hMatrix[i] = GLES20.glGetUniformLocation(shaderProgram[i],
						"u_mvp");
				hColor[i] = GLES20.glGetUniformLocation(shaderProgram[i],
						"u_color");
				hAlpha[i] = GLES20.glGetUniformLocation(shaderProgram[i],
						"u_alpha");
				hMode[i] = GLES20.glGetUniformLocation(shaderProgram[i],
						"u_mode");
				hVertexPosition[i] = GLES20.glGetAttribLocation(
						shaderProgram[i], "a_pos");
				hLightPosition[i] = GLES20.glGetAttribLocation(
						shaderProgram[i], "a_light");
			}

			ByteBuffer buf = ByteBuffer.allocateDirect(mBufferSize).order(
				...
				if (!el.compiled) {
					...
					if (verticesBytes > mBufferSize) {
						...
						ByteBuffer buf = ByteBuffer.allocateDirect(
								verticesBytes).order(ByteOrder.nativeOrder());
