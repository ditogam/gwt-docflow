...
public class TextOverlay extends BasicOverlay {
	...
	private static void addDebugBox(Layers dbg, Label l, TextItem ti,
			int overlaps, boolean prev, float scale) {
	...
	private byte checkOverlap(TextLayer tl, Label ti) {
		...
		for (Label lp = (Label) tl.labels; lp != null;) {
			...
			if (lp.text == ti.text
					&& (lp.string == ti.string || lp.string.equals(ti.string))) {
	...
	protected void setMatrix(MapPosition curPos, Matrices m) {
		...
		GlUtils.setMatrix(m.mvp, x * s, y * s, scale / GLRenderer.COORD_SCALE);
	...
	boolean updateLabels() {
		...
		for (l = mPrevLabels; l != null;) {
			...
			if (!l.text.caption) {
				...
				l.bbox.set(l.x, l.y, l.x1, l.y1, l.width + 5,
						l.text.fontHeight + 5);
		...
		for (int i = 0, n = mTileSet.cnt; i < n; i++) {
			...
			for (TextItem ti = t.labels; ti != null; ti = ti.next) {
				...
				if (l.bbox == null)
					l.bbox = new OBB2D(l.x, l.y, l.x1, l.y1, l.width + 5,
							l.text.fontHeight + 5);
				else
					l.bbox.set(l.x, l.y, l.x1, l.y1, l.width + 5,
							l.text.fontHeight + 5);
