...
public final class LineRenderer {
	...
	private final static String lineVertexShader = ""
			...
			+ "attribute vec4 a_pos;" + "uniform float u_mode;"
			+ "varying vec2 v_st;"
			+ "void main() {"
	...
	private final static String lineSimpleFragmentShader = ""
			...
			+ "uniform float u_wscale;" + "uniform float u_mode;"
			+ "uniform vec4 u_color;" + "varying vec2 v_st;" + "void main() {"
			// + "  float len;"
			// (currently required as overlay line renderers dont load the
			// texture)
			// + "  if (u_mode == 0)"
			// + "    len = abs(v_st.s);"
			// + "  else"
			...
			// where wscale is 'filter width' / 'line width' and 0 <= len <=
			// sqrt(2)
	...
	private final static String lineFragmentShader = ""
			...
			// smoothstep is too sharp, guess one could increase extrusion with
			// z..
			// this looks ok:
			// +
			// "  gl_FragColor = u_color * min(1.0, (1.0 - len) / (u_wscale + fuzz));"
			// can be faster according to nvidia docs 'Optimize OpenGL ES 2.0
			// Performace'
	...
	public static Layer draw(Layers layers, Layer curLayer, MapPosition pos,
		...
		glVertexAttribPointer(hLineVertexPosition[mode], 4, GL_SHORT, false, 0,
				layers.lineOffset + LINE_VERTICES_DATA_POS_OFFSET);
			...
			if (line.outline) {
				...
				for (LineLayer o = ll.outlines; o != null; o = o.outlines) {
					...
					if (o.line.fixed || strokeMaxZoom) {
						...
						if (o.line.min > 0
								&& o.width * lineScale < o.line.min * 2)
							continue;
					}
				...
				if (line.fixed || strokeMaxZoom) {
					...
					// very thin
					if ((ll.line.min > 0)
							&& (ll.width * lineScale < ll.line.min * 2))
	...
	static boolean init() {
		...
		for (int i = 0; i < 2; i++) {
			...
			hLineVertexPosition[i] = glGetAttribLocation(lineProgram[i],
					"a_pos");
		}
