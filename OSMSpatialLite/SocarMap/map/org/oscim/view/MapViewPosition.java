...
import org.oscim.utils.Mercator;
...
public class MapViewPosition {
	/**
	 * below is borrowed from CountDownTimer class: Copyright (C) 2008 The
	...
	int getPixelZoomLevel() {
		return this.getMapPosition().zoomLevel;
	}

	public Point toMapPixels(final GeoPoint in, final Point reuse) {
		final Point out = (reuse != null) ? reuse : new Point();
		int viewWidth_2 = mMapView.getWidth() / 2;
		int viewHeight_2 = mMapView.getHeight() / 2;
		int worldSize_2 = getWorldSizePx() / 2;
		int offsetX = -worldSize_2;
		int offsetY = -worldSize_2;
		final int[] coords = Mercator.projectGeoPoint(in.getLatitude(),
				in.getLongitude(), getPixelZoomLevel(), null);
		out.set(coords[Mercator.MAPTILE_LONGITUDE_INDEX],
				coords[Mercator.MAPTILE_LATITUDE_INDEX]);
	...
	Point geoPointToPoint(GeoPoint gp) {
		...
		double longitude = ((double) gp.getLongitude()) * Math.PI / 180.0;
		ret.x = (int) (radius * longitude + (circumference / 2.0));
		double latitude = ((double) gp.getLatitude()) * Math.PI / 180.0;
		ret.y = (int) ((circumference / 2.0) - (radius / 2.0 * Math
				.log((1.0 + Math.sin(latitude)) / (1.0 - Math.sin(latitude)))));
		return ret;
	}

	private void calculateCirc() {
		int tiles = 1 << getMapPosition().zoomLevel;
	...
	public Point toPixels(GeoPoint in, Point out) {
		...
		Point c = geoPointToPoint(mMapView.getCenter());
		out.set(p.x - c.x + (mMapView.getWidth() / 2),
				p.y - c.y + (mMapView.getHeight() / 2));
		return out;
	}
	...
	public synchronized void animateTo(BoundingBox bbox) {
		...
		double dx = Math.abs(MercatorProjection.longitudeToX(bbox
				.getMaxLongitude())
				- MercatorProjection.longitudeToX(bbox.getMinLongitude()));

		double dy = Math.abs(MercatorProjection.latitudeToY(bbox
				.getMinLatitude())
				- MercatorProjection.latitudeToY(bbox.getMaxLatitude()));

		double aspect = (Math.min(mWidth, mHeight) / Tile.TILE_SIZE);
		double z = Math.min(-LOG4 * Math.log(dx) + aspect, -LOG4 * Math.log(dy)
				+ aspect);
		...
		Log.d(TAG, "scale to " + bbox + " " + z + " " + newScale + " "
				+ mAbsScale + " " + FastMath.log2((int) newScale) + " " + scale);
	...
	/**
	 * Get the GeoPoint for x,y in screen coordinates. (only used by
	 * MapEventsOverlay currently)
	 * 
	 * @param x
	 *            screen coordinate
	 * @param y
	...
	public synchronized GeoPoint fromScreenPixels(float x, float y) {
		...
		GeoPoint p = new GeoPoint(MercatorProjection.toLatitude(dy),
	...
	public synchronized MapPosition getMapPosition() {
		...
		return new MapPosition(mLatitude, mLongitude, (byte) z, scale,
				mRotation);
	}
	 ...
	 * 
	 * @param pos
	 *            MapPosition object to be updated
	...
	public synchronized boolean getMapPosition(MapPosition pos) {
		...
		if (pos.lat == mLatitude && pos.lon == mLongitude && pos.zoomLevel == z
				&& pos.scale == scale && pos.angle == mRotation
				&& pos.tilt == mTilt)
			return false;
	...
	/**
	 * Get the inverse projection of the viewport, i.e. the coordinates with
	 * z==0 that will be projected exactly to screen corners by current
	...
	/**
	 * For x, y in screen coordinates set Point to map-tile coordinates at
	 * returned scale.
	 * 
	 * @param x
	 *            screen coordinate
	...
	/*
	 * Get Z-value of the map-plane for a point on screen - calculate the
	 * intersection of a ray from camera origin and the map plane
	 */
	 ...
	 *            the GeoPoint
	 * @param out
	 *            Point projected to screen pixel
	 */
	public synchronized void project(GeoPoint geoPoint, PointF out) {
		...
		double x = MercatorProjection.longitudeToX(geoPoint.getLongitude())
				* mCurScale;
		double y = MercatorProjection.latitudeToY(geoPoint.getLatitude())
				* mCurScale;
	...
	public void setListener(MapListener mListener) {
	...
	void setViewport(int width, int height) {
		...
		Matrix.frustumM(mProjMatrix, 0, -s, s, aspect * s, -aspect * s,
				VIEW_NEAR, VIEW_FAR);
	...
	private void unproject(float x, float y, float z, float[] coords,
			int position) {
		mv[0] = x;
		mv[1] = y;
		mv[2] = z;
	...
	private void updatePosition() {
		...
		if (mListener != null) {
			mListener.onStateChanged(getMapPosition());
		}
	}
}
