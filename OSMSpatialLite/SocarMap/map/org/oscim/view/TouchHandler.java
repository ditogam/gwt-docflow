 ...
 * @TODO: - use one AnimationTimer instead of CountDownTimers - fix recognition
 *        of tilt/rotate/scale state...
 */
...
final class TouchHandler implements OnGestureListener, OnDoubleTapListener {
	...
	public boolean onDoubleTap(MotionEvent e) {
		...
		mDoubleTap = true;
		mMapPosition
				.setZoomLevel((byte) (mMapPosition.getMapPosition().zoomLevel + 1));
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
		...
		// return true;
		//
		// float move = Math.min(mMapView.getWidth(), mMapView.getHeight()) * 2
		// / 3;
		// mMapPosition.animateTo(vx * move, vy * move, 250);
		// } else {
		float s = (200 / mMapView.dpi);

		mMapPosition.animateFling(Math.round(velocityX * s),
				Math.round(velocityY * s), -w, w, -h, h);
		return true;
	}
	...
	private void printState(String action) {
		Log.d(TAG, action + " " + mDoubleTap + " " + mBeginScale + " "
				+ mBeginRotate + " " + mBeginTilt);
	}
