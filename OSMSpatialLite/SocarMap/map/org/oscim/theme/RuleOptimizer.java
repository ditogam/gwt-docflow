...
final class RuleOptimizer {
		...
		for (int i = 0, n = ruleStack.size(); i < n; ++i) {
			if (ruleStack.get(i) instanceof PositiveRule) {
				...
				if (positiveRule.mKeyMatcher != null
						&& positiveRule.mKeyMatcher
								.isCoveredBy(attributeMatcher)) {
					return null;
				}
			}
		}
	...
	// static ClosedMatcher optimize(ClosedMatcher closedMatcher, Stack<Rule>
	// ruleStack) {
	...
	// }
	//
	// static ElementMatcher optimize(ElementMatcher elementMatcher, Stack<Rule>
	// ruleStack) {
	//
