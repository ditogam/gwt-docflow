...
public final class MercatorProjection {
	 ...
	 */
	public static double calculateGroundResolution(double latitude,
			byte zoomLevel) {
	...
	public static double wrapLongitude(double longitude) {
		if (longitude < -180)
			return Math.max(Math.min(360 + longitude, LONGITUDE_MAX),
					LONGITUDE_MIN);
		else if (longitude > 180)
			return Math.max(Math.min(longitude - 360, LONGITUDE_MAX),
					LONGITUDE_MIN);
	...
	//
	// out.x = (int)
	// MercatorProjection.longitudeToPixelX(geopoint.getLongitude(), z);
	// out.y = (int) MercatorProjection.latitudeToPixelY(geopoint.getLatitude(),
	// z);
	//
	// return out;
	// }
