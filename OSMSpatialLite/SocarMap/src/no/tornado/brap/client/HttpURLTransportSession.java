...
public class HttpURLTransportSession implements TransportSession {
	...
	@Override
	public InputStream sendInvocationRequest(Method method,
			InvocationRequest request, InputStream streamArgument)
			throws IOException {
		conn = (HttpURLConnection) new URL(invocationHandler.getServiceURI())
				.openConnection();
		...
		if (streamArgument != null)
			sendStreamArgumentToHttpOutputStream(streamArgument,
					conn.getOutputStream());

		return conn.getInputStream();
	}

	private void sendStreamArgumentToHttpOutputStream(
			InputStream streamArgument, OutputStream outputStream)
			throws IOException {
