...
/**
 * Internal class for dumping an entire database. It contains a special callback
 * interface to traverse the tables of the current database and output create
 * SQL statements and for the data insert SQL statements.
 */
				...
				if (s2.db.is3()) {
					...
					if (t != null) {
						...
						for (int i = 0; i < t.nrows; i++) {
							String col = t.rows.elementAt(i)[1];
...
/**
 * SQLite command line shell. This is a partial reimplementaion of
 * sqlite/src/shell.c and can be invoked by:
 * <P>
 * 
 * <verb> java SQLite.Shell [OPTIONS] database [SHELLCMD] or java -jar
 * sqlite.jar [OPTIONS] database [SHELLCMD] </verb>
 */
...
public class Shell implements Callback {
	...
	void do_meta(String line) {
		...
		if (cmd.compareTo(".tables") == 0) {
			...
			if (t != null) {
				for (i = 0; i < t.nrows; i++) {
					String tab = t.rows.elementAt(i)[0];
					if (tab != null) {
						pw.println(tab);
					}
				}
