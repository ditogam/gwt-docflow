...
import com.spatialite.utilities.Balance;
import com.spatialite.utilities.Customer;
import com.spatialite.utilities.Meter;
...
public class DBLoader {
	...
	private DBLoader(String dbPath) throws Exception {
		...
		try {
			...
			getMap(mDatabase, "SELECT  distinct subregionid,zone FROM zones",
					1, 1, 0, null, zones);
			getMap(mDatabase,
	...
	public ArrayList<Customer> getCustomers(Long subregion_id, Long zone,
		...
		try {
			mDatabase.open(dbPath, jsqlite.Constants.SQLITE_OPEN_READONLY);
			String sql = "SELECT c.cusid,case when building_id is null then 0 else building_id end building_id ,cusname||' ('||cityname ||'-' || streetname ||'/' ||flat||'/' ||home||')' cusname \n"
